...
exports.buscarClientesSP = buscarClientesSP;
exports.modeloLogico = modeloLogico;
const bancoDeDados = {
    Cliente: [
...
function buscarClientesSP() {
    const estadoSP = bancoDeDados.Estado.find((estado) => estado.sigla === "SP");
    if (!estadoSP) {
        console.log("Estado não encontrado.");
        return;
    }
    const clientesSP = bancoDeDados.Cliente.filter((cliente) => cliente.estado_id === estadoSP.estado_id);
    console.log(`Clientes do estado: ${estadoSP.nome} (${estadoSP.sigla}):\n`);
    clientesSP.forEach((cliente) => {
        const telefones = bancoDeDados.Telefone.filter((telefone) => telefone.cliente_id === cliente.cliente_id);
        console.log(`Cliente ID: ${cliente.cliente_id}, Razão Social: ${cliente.razao_social}`);
